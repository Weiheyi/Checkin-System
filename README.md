...
- 学习工具：计时器、倒计时（到点响铃）、离线英汉词典（音标 + 中文释义）
...
由你选好题型与题量、再点「开始考核」（默认只随机抽 10 题，想把这些词全考就把题量改成「全部」）。
在设置页改动「范围」会取消这次选中。
...
- 先归一化：去掉词性标记（`vt.`、`n.` 等）、括号注解、标点与空白。
- 释义按 `；，、|/` 拆成多个义项，命中**任意一个**义项就算对。
- 命中规则依次是：完全一致 → 互相包含（答「苹果」对「苹果树」）→ 相似度（公共子序列占比）≥ 0.8。
- 单字释义（如「是」）只认完全一致，避免「不是」被判成对。

中文写法多样，自动判分**做不到百分之百准确**（例如近义词「抛弃 / 放弃」判不出来）。
因此自动判错时**不会立刻计分**，而是给一个 **「我答对了，算对」** 按钮：
点了就按答对重新计分；不点、直接点「下一题」才按答错落账。这样一次误判不会污染熟练度。
