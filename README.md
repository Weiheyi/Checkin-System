...
- 学习工具：计时器、倒计时（到点响铃）、离线字典（内置 12.5 万词美音音标）
...
工具页的「🔤 字典」是一个**纯离线**的音标查询：输入英文单词，下面立刻显示它的美音音标，
点旁边的 🔊 可以朗读（朗读用的是浏览器自带的语音合成，见下一节）。

- 数据就是内置的 `public/vendor/ipa/en_US.txt`（125,927 条，约 2.9MB），
  **第一次打开字典页时加载一次，之后完全离线，不再联网**。
- 输入即查，不需要点按钮；也会自动去掉括号、词性等修饰再查一次。
- 词典没收录的词（多为生僻词、专有名词）会直接提示「没有收录」，不影响查别的词。
- 在输入框里按回车可以直接朗读当前单词。

> 注意：这个字典**只有音标和发音，没有中文释义**——仓库里没有内置英汉释义数据。
> 需要中文释义的话，用「背单词」里自己导入的单词本。
